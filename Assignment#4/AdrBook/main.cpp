...
{
	string temp;
	vector<string> args, arglist = { "<name>", "<album>" , "<artist>", "<genre>" };
	ifstream in(file);
	if (!in.is_open())
		return;
	...
	while (in.getline(buffer, 256))
	{
		...
		for (string s : arglist)
		{
			...
			{
				args.push_back(temp.substr(temp.find_first_of('>') + 1, temp.find_last_of('<') - temp.find_first_of('>') - 1));
				break;
			}
		}
	}
...
void musicListSave(string file, HWND& hwnd)
{
	...
	FOREACH_LIST(musicList, i, info)
	{
		...
		out << "\t\t<name>" << info.album << "</name>" << endl;
		out << "\t\t<name>" << info.artist << "</name>" << endl;
		out << "\t\t<name>" << info.genre << "</name>" << endl;
		out << "\t</music>" << endl;
	}
	out << "</musicInfo>" << endl;
